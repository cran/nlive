...
#' containing the results of the fit of the data by the PMM-abrupt. The _nlive.pmma_ function
...
nlive.pmma <- function(dataset, ID, outcome, time, var.all = NULL,
                       var.last.level = NULL,
                       var.slope1 = NULL,
                       var.slope2 = NULL,
  ...
  if (!is.null(traj.marg.group.val) & is.null(var.all) & is.null(var.last.level) & is.null(var.slope1) & is.null(var.slope2) & is.null(var.changepoint))
    ...
    if(traj.marg == TRUE){
      ...
      tab_traj$traj_PMM = tab_traj$I1*(n0 + n2*n3 + n1*(tab_traj$time - n3)) + tab_traj$I2*(n0 + n2*tab_traj$time)
